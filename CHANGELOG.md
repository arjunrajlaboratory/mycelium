...
- **Lifecycle ownership, locking, and notebook attribution.** Active sessions now publish a distinct host-session owner token, so a nested subagent Stop cannot consolidate, finalize, or delete its primary session's shared lifecycle state; malformed or missing new-format identity fails closed, while timestamp comparison remains as an upgrade-compatible fallback for already-active legacy sessions. Recent lifecycle locks whose recorded owner process has terminated are reclaimed immediately, while ownerless locks retain protection for the publication race; if a live owner remains busy through the retry budget, Stop now blocks instead of silently bypassing lifecycle enforcement. Jupyter lineage parsing now consumes known separated option values such as `--to` and `--output`, preventing option values from hiding or impersonating the true notebook input, and rejects terminal configuration modes such as `--show-config` and `--generate-config` anywhere in the matched simple command.
