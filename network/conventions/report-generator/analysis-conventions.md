...
**Drafting model — draft with Sonnet, verify with the orchestrator.** Dispatch the Phase-2 draft pass — and the Phase-2 patch passes that respond to Phase 4/5/6 findings — to a **Sonnet sub-agent** (`model: claude-sonnet-4-6`; confirm the latest Sonnet model ID at runtime). The comparison that motivated this skill found Sonnet writes more natural prose, so the writing work goes to the more natural writer. Everything else stays on the orchestrating model: the manifest build (Phase 1), the worked-example gate (Phase 3), the blind reviewers (Phases 4–6), and the recompile gate (Phase 7). This carries no correctness risk — the blind numerical re-verify (Phase 6) is unchanged and still catches any number or label error the drafter introduces, so delegating *prose* to Sonnet cannot quietly corrupt a *value*. In Claude Code, the orchestrator dispatches drafting via the `Agent` tool with `model: sonnet`; in a Workflow, via `agent(prompt, {model: 'sonnet'})`.
...
- **Display faithfulness (derived-number check).** For each `numbers[*]` entry carrying a `unit` or `display` field, confirm the rendered macro is a faithful function of the canonical `value`: a `unit:"percent"` macro must equal `value*100` at the entry's `precision` (e.g. `0.978 → 97.8\%`), and a free-text `display` must not state a number that contradicts `value`. A display that does not derive from `value` is the one way the readable number can drift from the verified one — flag it as a provenance finding (major when it changes the number a reader takes home).
