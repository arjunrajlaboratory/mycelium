...
7. **After writing each finding**, upsert a row in `.living/findings/FINDINGS_REGISTRY.md`. If the file doesn't exist, create it from `skills/core/templates/findings-registry.md`. Then upsert the row with `python3 skills/core/scripts/upsert_manifest_row.py .living/findings/FINDINGS_REGISTRY.md <F-NNN> "<row>"` instead of hand-editing — it locks and writes atomically, so concurrent chats can't clobber each other's rows (matches on finding ID F-NNN: replaces the row if present, else appends). Columns: ID, Claim, Status, Topic (link to topic file), Implications, Tags, Last Updated. See `skills/core/references/concurrency.md`.
